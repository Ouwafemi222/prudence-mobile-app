-- Lifetime tag registry per user per office (case-insensitive); multiple tags allowed per day
...
DROP POLICY IF EXISTS "Users can view own submission tags" ON public.user_submission_tags;
CREATE POLICY "Users can view own submission tags"
  ON public.user_submission_tags FOR SELECT TO authenticated
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Admins can view office submission tags" ON public.user_submission_tags;
...
DROP POLICY IF EXISTS "Users can insert own submission tags" ON public.user_submission_tags;
