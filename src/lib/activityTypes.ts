...
export function gigLinksFrom(activity: {
  ...
  return proofPathsFrom(activity.gig_links, activity.gig_link).filter(Boolean);
}

export function accountLinksFrom(activity: { account_links?: string[] | null }): string[] {
...
export function getActivityProofSections(
  ...
  return {
    ...
    other: proofPathsFrom(activity.other_activities_proof_images, activity.other_activities_proof_image),
  };
}
